...
def create_check_genesis_and_split_prompt():
    prompt = PromptTemplate.from_template(
        ...
        4. 질문이 일반적인 차량 사용에 관련된 질문(예: 엔진 오일 교체, 타이어 공기압 확인, 연비 등)이라면 **제네시스 차량 질문으로 간주**하세요. 이 경우 제네시스 차량과 관련된 구체적인 정보를 바탕으로 답변할 준비를 합니다.
...
def create_history_base_answer_prompt():
    prompt = PromptTemplate.from_template(
        """
        다음은 이전 대화 내용만을 참고하여 응답을 생성하기 위한 프로세스입니다. 
        외부 지식이나 학습된 데이터를 사용하지 않고 오직 제공된 대화 내용을 기반으로 응답합니다.
        ...
        ## 절차
        1. **이전 대화 분석**: CHAT_HISTORY을 검토하여 대화의 주제 및 구체적으로 제기된 질문을 파악합니다.
        2. **질문 또는 프롬프트 식별**: 명시적 또는 암시적인 질문이나 요청을 CHAT_HISTORY에서 식별합니다.
        3. **응답 가능성 평가**: 외부의 지식, 학습된 데이터, 일반 상식 등을 사용하지 않고 제공된 CHAT_HISTORY 만으로 충분한 정보가 있는지 판단하고, 응답 가능 여부를 결정합니다.
           - 응답 가능하면 `'yes'`를 출력합니다.
           - 응답이 불가능하면 `'no'`를 출력합니다.
        4. **응답 생성**: 
           - 가능(`'yes'`)하면 CHAT_HISTORY 만을 바탕으로 논리적이고 맥락에 맞는 응답을 작성합니다.
           - 불가능(`'no'`)하면 추가 정보가 필요하다는 점을 명시합니다.

        ## 제한 사항
           - CHAT_HISTORY 외부의 지식, 학습된 데이터, 일반 상식 등을 사용하지 않습니다.
           - CHAT_HISTORY에서 AI의 응답만을 참고하며, 인간 메시지를 단순히 분석에 활용합니다.
           - CHAT_HISTORY에서 AI의 응답에 명확한 맥락이나 정보가 부족하면 응답하지 않습니다.
